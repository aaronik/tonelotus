...
(function(root){
	...
	State.load = function(query){
		...
		setTimeout(function(){
			ToneLotus.router.drawMatrix(ToneLotus.router.currentMatrix);
		}, 2000)
	};
  ...
  State.trackMatrix = function(matrix, trackNumber){
	...
	State.generateLoopTime = function(){
		var urlString = 'lt=' + (240 / ToneLotus.Metronome.totalLoopTime) * 1000; // that's bpm
		return urlString;
	};
	...
	State.generateTracks = function(){
		...
		for(var trackNumber = 1; trackNumber <= 3; trackNumber++){
			...
			$('#track' + trackNumber).children('ul').children('li').children().each(function(i, matrix){
				currentMatrix = ToneLotus.Store.getMatrixByCID($(matrix).attr('data-cid'));
				index = ToneLotus.Store.matrixArray.indexOf(currentMatrix);
				if( !currentMatrix ){ index = 'b' }
				urlString += index + ',';
			})
